...
- Tích hợp theo tuần bằng merge commit, không squash và không xóa nhánh thành viên; giáo viên vẫn có thể chấm riêng theo branch, tag và lịch sử tác giả.
- Không merge trực tiếp nhánh thành viên vào `main`. Mỗi tuần tạo `integration/week-XX` từ `main`, kiểm tra hợp nhất tại đó rồi mới merge một lần vào `main`.
...
- Review contract giữa bốn module và chỉ đưa code thành viên qua cổng `integration/week-XX` đã quy định.
...
- Chuẩn bị checklist tích hợp và dùng nhánh `integration/week-08` theo quy trình merge hằng tuần.
...
### Lịch merge để tránh xung đột tích lũy

- Thứ Năm 18:00: feature freeze; mỗi thành viên push commit chạy được, cập nhật trạng thái và gắn tag `member/week-XX` hoặc cung cấp commit SHA.
- Thứ Sáu: Hiếu review diff, contract, migration và test; phần không đạt giữ lại nhánh thành viên, không chặn các module độc lập khác.
- Thứ Bảy: tạo `integration/week-XX` từ `main` mới nhất và merge bằng `--no-ff` theo thứ tự `main/Hiếu → Bo/Auth → Trường/Recipe-File → Kiên/Search-Job`.
- Sau mỗi nhánh được nhập vào integration, chạy backend build/test. Sau nhánh cuối, regenerate EF Core snapshot/migration tích hợp, chạy frontend checks, Compose và smoke test.
- Chủ Nhật: chỉ khi toàn bộ cổng chất lượng đạt mới merge `integration/week-XX` vào `main` bằng merge commit và push một lần.
- Thứ Hai tuần kế tiếp: từng thành viên merge `origin/main` trở lại nhánh cá nhân; không rebase/force-push nhánh đã công bố để tránh đổi lịch sử dùng cho chấm điểm.

Không đợi đến cuối đồ án mới merge. Mốc nội dung dự kiến:

- Cuối Tuần 1: shared contracts, Identity/Recipe schema đã sửa, contract Search/Job của Kiên; không nhận default secret hoặc contract idempotency chưa rõ retry.
- Cuối Tuần 2: Category/Auth/Recipe API lõi và Search filter/sort/pagination không FTS.
- Cuối Tuần 3: vòng đời Auth/Recipe, Category backend và PostgreSQL FTS/migration.
- Cuối Tuần 4: RBAC/resource authorization, nested Recipe và các UI vòng một.
- Cuối Tuần 5: Google Login, MinIO, observability và Hangfire infrastructure.
- Cuối Tuần 6: frontend nghiệp vụ và ba background job thật; đây là mốc feature-complete.
- Cuối Tuần 7: chỉ merge test, hardening, performance và bug fix; không mở rộng chức năng.
- Cuối Tuần 8: merge sửa lỗi liên-module từ nhánh integration; khóa API contract và migration chain.
- Cuối Tuần 9: tạo release candidate trên `main` sau E2E/security/performance.
- Cuối Tuần 10: chỉ nhận bug fix được review, tạo release tag cuối và đóng băng mã nguồn.

Một nhánh chỉ được nhập vào integration khi build/test xanh, không có secret, migration chạy từ database trắng và từ database của tuần trước, API contract không phá vỡ module đã merge, và chức năng chưa hoàn chỉnh được bảo vệ bằng feature flag hoặc chưa được expose. Nếu một module trượt cổng, ghi rõ SHA bị hoãn và chuyển sang lần merge kế tiếp; không sửa vội trực tiếp trên `main`.
...
- Dùng Draft Pull Request để review nhánh thành viên; khi đạt cổng tuần, merge qua `integration/week-XX` bằng merge commit để giữ lịch sử chấm riêng.
